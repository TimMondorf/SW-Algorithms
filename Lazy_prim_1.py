...
class Lazy_prim:
    ...
    def __init__(self, graph):
        ...
        while False in self.marked:
            ...
            self.possible_next_edge = self.q_of_e[self.min_index]
            del self.q_of_e[self.min_index]
            del self.q_of_e_w[self.min_index]
            if self.marked[self.possible_next_edge.head] == False or self.marked[self.possible_next_edge.tail] == False:
                self.MST.append(self.possible_next_edge)
                self.total_weight += self.possible_next_edge.weight
                if self.marked[self.possible_next_edge.head] == False:
                    self.next_vertex = self.possible_next_edge.head
                elif self.marked[self.possible_next_edge.tail] == False:
                    self.next_vertex = self.possible_next_edge.tail
...
while i < 99:
    ...
    i += 1
tim_graph_find_mst = Lazy_prim(tim_graph)
